...
const createUsernames = (devs) => {
	return devs.map((dev) => {
		const nameString =
			dev.firstName.toLowerCase() + dev.lastName[0].toLowerCase()
		const yearString = new Date().getFullYear() - dev.age
		dev.username = nameString.concat(yearString)
		return dev
	})
}
...
console.log(createUsernames(list1))
